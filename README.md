...
При первом применении миграций автоматически создаётся пользователь:

```text
username: admin
```

Пользователь `admin` получает права администратора (`is_admin`, `is_staff`, `is_superuser`), но создаётся без рабочего пароля. Это исключает хранение стандартного административного пароля в публичном репозитории.

Для создания администратора с возможностью входа в приложение рекомендуется использовать команду:
...
Перед выполнением команды необходимо задать `ADMIN_INITIAL_PASSWORD` в `.env`.

Также администратора можно создать стандартной командой Django:

```bash
docker compose exec web python manage.py createsuperuser
```

Пароли администраторов не должны храниться в исходном коде или публиковаться в GitHub.
...
Максимальный размер одного загружаемого файла — **100 МБ**. Ограничение проверяется на стороне backend.
