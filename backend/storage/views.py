...
def is_admin(user):
    return bool(user.is_authenticated and (getattr(user, 'is_admin', False) or user.is_superuser))
...
class FileListUploadView(APIView):
    ...
    def post(self, request):
        ...
        if len(files) > settings.MAX_FILES_PER_UPLOAD:
            return Response({'detail': f'За один раз можно загрузить не более {settings.MAX_FILES_PER_UPLOAD} файлов.'}, status=status.HTTP_400_BAD_REQUEST)
        oversized = [f.name for f in files if f.size > settings.MAX_UPLOAD_FILE_SIZE]
        if oversized:
            return Response({'detail': f'Превышен максимальный размер файла: {oversized[0]}'}, status=status.HTTP_413_REQUEST_ENTITY_TOO_LARGE)
        ...
        for uploaded in files:
            ...
            try:
                ...
                return Response({'original_name': [str(getattr(exc, 'detail', exc))]}, status=status.HTTP_400_BAD_REQUEST)
            item = File.objects.create(
                user=target_user, original_name=original_name, file=uploaded, size=uploaded.size, comment=comment,
            )
            created.append(item)
            logger.info('Пользователь %s загрузил файл %s (ID %s) в хранилище %s', request.user.username, item.original_name, item.id, target_user.username)
...
class FileDetailView(APIView):
    def patch(self, request, pk):
        ...
        if item is None:
            return Response({'detail': 'Файл не найден или доступ запрещен'}, status=status.HTTP_404_NOT_FOUND)
        serializer = FileSerializer(item, data=request.data, partial=True, context={'request': request})
        serializer.is_valid(raise_exception=True)
        serializer.save()
        logger.info('Файл ID %s обновлен пользователем %s', item.id, request.user.username)
    ...
    def delete(self, request, pk):
        ...
        item.delete()
        logger.info('Файл %s (ID %s) удален пользователем %s', original_name, pk, request.user.username)
...
class FileShareView(APIView):
    def post(self, request, pk):
        ...
        if item is None:
            return Response({'detail': 'Файл не найден или доступ запрещен'}, status=status.HTTP_404_NOT_FOUND)
        return Response({'special_link': str(item.share_token), 'public_url': request.build_absolute_uri(f'/share/{item.share_token}')})
...
class PublicFileInfoView(APIView):
    ...
    def get(self, request, share_token):
        try:
            ...
            return Response({'detail': 'Файл по данной ссылке не найден'}, status=status.HTTP_404_NOT_FOUND)
