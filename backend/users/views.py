...
from users.serializers import AdminUserUpdateSerializer, UserRegisterSerializer, UserSerializer
...
class AdminUserDetailView(APIView):
    ...
    def patch(self, request, pk):
        ...
        if user.pk == request.user.pk and request.data.get('is_admin') is False:
            return Response({'detail': 'Нельзя снять права администратора у самого себя'}, status=status.HTTP_400_BAD_REQUEST)
        serializer = AdminUserUpdateSerializer(user, data=request.data, partial=True)
        serializer.is_valid(raise_exception=True)
        serializer.save()
        logger.info('Администратор %s изменил права пользователя %s', request.user.username, user.username)
        return Response(UserSerializer(user).data)
