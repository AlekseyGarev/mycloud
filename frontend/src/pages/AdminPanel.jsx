...
export default function AdminPanel() {
  ...
  const toggleAdmin = async (u) => {
    try {
      const r = await api.patch(`/admin/users/${u.id}/`, {
        is_admin: !u.is_admin,
      });
      setUsers((xs) => xs.map((x) => (x.id === u.id ? r.data : x)));
    } catch (e) {
      alert(e.response?.data?.detail || "Не удалось изменить права");
    }
  };
  const removeUser = async (u) => {
    if (!window.confirm(`Удалить пользователя ${u.username} и его файлы?`))
      return;
    try {
