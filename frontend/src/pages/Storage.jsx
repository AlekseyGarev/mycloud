...
export default function Storage() {
  ...
  const handleUpload = async (e) => {
    ...
    const result = await dispatch(
      uploadFile({ formData, userId: targetUserId })
    );
    ...
    try {
      ...
      setGeneratedLink(
        response.data.public_url ||
          `${window.location.origin}/share/${response.data.special_link}`
      );
    } catch {
      alert("Ошибка при получении публичной ссылки");
    }
  };
  ...
  return (
    <div className="container">
      <h2 className="home-title">
        {targetUserId
          ? `Хранилище пользователя ${requestedUsername || `#${targetUserId}`}`
      ...
      <div className="file-upload-card">
        ...
        <form onSubmit={handleUpload} className="mt-15">
          <div className="upload-grid">
            ...
            <div className="form-group">
              <label>Имя файла (для одного файла):</label>
              <input
                type="text"
                className="form-input"
                value={customName}
      ...
      {generatedLink && (
        <div className="share-link-box mb-15">
          <span>Специальная ссылка:</span>
          <input className="share-input" value={generatedLink} readOnly />
          <button onClick={handleCopy} className="btn btn-success btn-sm">
            Скопировать
          </button>
        </div>
      )}
          ...
          <tbody>
            {loading ? (
              ...
              files.map((file) => (
                <tr key={file.id}>
                  <td>
                    {editingId === file.id && editingField === "name" ? (
                      <input
                        className="form-input"
                  ...
                  <td>
                    {editingId === file.id && editingField === "comment" ? (
                      <input
                        className="form-input"
                        value={editComment}
